...
//!
//! Contains Iced GUI components and views with comprehensive contact field
//! support, alphabetical pagination, and multiple value fields (emails, phones,
//! URLs).
    ...
    fn to_contact(&self, id: Option<Uuid>) -> Result<Contact, String> {
        ...
        for date_form in &self.dates {
            if !date_form.date.trim().is_empty() {
                ...
                {
                    let contact_date =
                        ContactDate::new(naive_date, date_form.label.trim().to_string());
...
pub enum Message {
    ...
    FetchPhotos(Uuid),
    // (current, total)
    PhotoFetchProgress(usize, usize),
    PhotosFetchComplete,
    ClearFetchedPhotos,
    SelectPhoto(usize),
    SavePhotoToContact,
...
fn new() -> (State, Task<Message>) {
    ...
    let workspace_manager =
        WorkspaceManager::new(workspaces_dir).expect("Failed to create workspace manager");
    let state = State {
        workspace_manager,
...
fn update(state: &mut State, message: Message) -> Task<Message> {
    match message {
        Message::LoadWorkspaces => {
            ...
            Task::perform(
                async move { manager.load_workspaces().map_err(|e| e.to_string()) },
        ...
        Message::SelectWorkspace(workspace_id) => {
            ...
            if let Some(mut workspace) = workspace {
                ...
                Task::perform(
                    async move { Ok(VcfRepository::new(vcf_path)) },
        ...
        Message::ImportVcfAsWorkspace => {
                ...
                .pick_file()
            {
                ...
                Task::perform(
                    async move {
                        manager
                            .create_workspace(filename, path)
                            .map_err(|e| e.to_string())
                    },
                    |result| match result {
        ...
        Message::FetchPhotos(contact_id) => {
            if let Some(contact) = state.contacts.iter().find(|c| c.id == contact_id) {
                if contact.urls.is_empty() {
                    state.error_message =
                        Some("No social media URLs configured in this contact".to_string());
                    return Task::none();
                }
                ...
                Ok(contact) => {
                    if let Some(ref mut repo) = state.repository {
                        match repo.create(contact.clone()) {
                            ...
                            Err(err) => {
                                state.error_message =
                                    Some(format!("Failed to save contact: {}", err));
                            }
                        }
                    } else {
                ...
                Ok(contact) => {
                    if let Some(ref mut repo) = state.repository {
                        match repo.update(contact.clone()) {
                            Ok(_) => {
                                if let Some(idx) =
                                    state.contacts.iter().position(|c| c.id == contact.id)
                                {
                            ...
                            Err(err) => {
                                state.error_message =
                                    Some(format!("Failed to update contact: {}", err));
                            }
                        }
                    } else {
                        state.error_message = Some("No workspace selected".to_string());
            ...
            match result {
                Ok(contacts) => {
                    if let Some(ref mut repo) = state.repository {
                        ...
                        state.error_message =
                            Some(format!("✅ Successfully imported {} contacts", imported));
                    }
                }
                Err(err) => {
        ...
        for workspace in &state.workspaces {
            let workspace_card = container(
                column![
                    ...
                    text(format!(
                        "Last accessed: {}",
                        workspace.last_accessed.format("%Y-%m-%d %H:%M")
                    ))
                    .size(12),
                    row![
...
fn view_list(state: &State, letter_filter: Option<char>) -> Element<'_, Message> {
    ...
    alphabet_row = alphabet_row.push(button("All").on_press(Message::FilterByLetter(None)).style(
        if letter_filter.is_none() {
            button::primary
        } else {
            button::secondary
        },
        ...
        .filter(|contact| {
            ...
            let matches_letter = if let Some(letter) = letter_filter {
                contact
                    ...
                    .chars()
                    .next()
                    .map(|c| c.to_uppercase().next() == Some(letter))
                    .unwrap_or(false)
            } else {
                true
            };
    ...
    pagination_row = pagination_row.push(text(format!(
        "Page {} of {}",
        state.current_page + 1,
        total_pages.max(1)
    )));
    ...
    column![
        header,
        search_input,
        scrollable(alphabet_row).direction(scrollable::Direction::Horizontal(
...
fn render_contact_form(state: &State, contact_id: Option<Uuid>) -> Element<'_, Message> {
    let title = text(if contact_id.is_some() {
        "Edit Contact"
    } else {
        "Add New Contact"
    })
    ...
    form_content =
        form_content.push(text("Note: First URL will be used as profile picture source").size(12));
    ...
    for (i, addr) in state.form.addresses.iter().enumerate() {
        ...
        form_content = form_content.push(container(addr_section).padding(10).style(|_theme| {
            container::Style {
                background: Some(iced::Background::Color(iced::Color::from_rgb(
                    0.97, 0.97, 0.97,
                ))),
    ...
    // Significant Dates Section Significant Dates Section
    ...
    form_content = form_content.push(button("+ Add Date").on_press(Message::AddDate).padding(10));
    form_content = form_content.push(
        row![
    ...
    form_content =
        form_content.push(text("Add any additional information as key-value pairs").size(12));
    ...
    // Note: Iced's text_input doesn't support multiline natively, but we can make it
    // visually larger
    form_content = form_content.push(
        container(
    ...
    form_content = form_content.push(row![save_button, cancel_button].spacing(10));
    column![
...
fn view_detail(state: &State, contact_id: Uuid) -> Element<'_, Message> {
    ...
    if let Some(nickname) = contact.custom_fields.get("nickname") {
        if !nickname.is_empty() {
            details =
                details.push(row![text("Nickname:").size(14), text(nickname).size(14)].spacing(10));
        }
    }
    if let Some(birthday) = contact.custom_fields.get("birthday") {
        if !birthday.is_empty() {
            details =
                details.push(row![text("Birthday:").size(14), text(birthday).size(14)].spacing(10));
        }
    }
    ...
    for (key, value) in &contact.custom_fields {
        if key.starts_with("email_") && !value.is_empty() {
            details =
                details.push(row![text("📧 Email:").size(14), text(value).size(14)].spacing(10));
        }
    }
    if let Some(phone) = &contact.phone {
    ...
    for (key, value) in &contact.custom_fields {
        if key.starts_with("phone_") && !value.is_empty() {
            details =
                details.push(row![text("📱 Phone:").size(14), text(value).size(14)].spacing(10));
        }
    }
    ...
    for (key, value) in &contact.custom_fields {
        if key.starts_with("url_") && !value.is_empty() {
            ...
            details =
                details.push(row![text("🌐 URL:").size(14), text(value).size(14)].spacing(10));
        }
    }

    // Work info
    if contact.organization.is_some()
        || contact.title.is_some()
        || contact.custom_fields.get("department").is_some()
    {
        details = details.push(text("Work Information").size(16));
        if let Some(org) = &contact.organization {
            details = details
                .push(row![text("🏢 Organization:").size(14), text(org).size(14)].spacing(10));
        }
        if let Some(title_text) = &contact.title {
            details = details
                .push(row![text("💼 Title:").size(14), text(title_text).size(14)].spacing(10));
        }
        if let Some(dept) = contact.custom_fields.get("department") {
            if !dept.is_empty() {
                details = details
                    .push(row![text("🏛️ Department:").size(14), text(dept).size(14)].spacing(10));
            }
        }
    }
    ...
    if !contact.urls.is_empty() {
        ...
        for url_obj in &contact.urls {
            let label_text = url_obj
                .label
                .as_ref()
                .map(|l| format!("{}: ", l))
                .unwrap_or_default();
            details = details.push(text(format!("  {}{}", label_text, url_obj.url)).size(13));
        }
    ...
    for (_i, (key, value)) in contact.custom_fields.iter().enumerate() {
        if key.starts_with("address_") && key.ends_with("_street") && !value.is_empty() {
            ...
            let addr_idx = key
                .strip_prefix("address_")
                .and_then(|s| s.strip_suffix("_street"))
            ...
            if let Some(idx) = addr_idx {
                ...
                if let Some(postal) = contact
                    .custom_fields
                    .get(&format!("address_{}_postal_code", idx))
                {
                    if !postal.is_empty() {
                ...
                if let Some(country) = contact
                    .custom_fields
                    .get(&format!("address_{}_country", idx))
                {
                    if !country.is_empty() {
    ...
    for (key, value) in &contact.custom_fields {
        if key.starts_with("date_") && !key.ends_with("_label") && !value.is_empty() {
            ...
            let date_idx = key
                ...
                .and_then(|s| s.parse::<usize>().ok());
            let label =
                date_idx.and_then(|idx| contact.custom_fields.get(&format!("date_{}_label", idx)));
            if let Some(label_text) = label {
                details = details.push(text(format!("📅 {}: {}", label_text, value)).size(14));
            } else {
                details = details.push(text(format!("📅 Date: {}", value)).size(14));
            }
        }
    }
    ...
    for (key, value) in &contact.custom_fields {
        ...
        if !key.starts_with("email_")
            && !key.starts_with("phone_")
            && !key.starts_with("url_")
            && !key.starts_with("address_")
    ...
    if !user_custom_fields.is_empty() {
        ...
        for (key, value) in user_custom_fields {
            details = details
                .push(row![text(format!("{}:", key)).size(14), text(value).size(14)].spacing(10));
        }
    }
    ...
    details = details.push(
        text(format!(
            "Updated: {}",
            contact.updated_at.format("%Y-%m-%d %H:%M:%S")
        ))
        .size(12),
    );
    column![
