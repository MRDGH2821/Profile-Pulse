...
use profile_pulse_core::{
    Contact, ContactId, EmailAddress, Profile, ProfileId, ProfileSettings, contact_to_vcard_bytes,
};
use profile_pulse_storage::{
    ContactIndex, ContactService, FsVdirBackend, SqliteContactIndex, StorageBackend,
};
use std::sync::Arc;
...
async fn import_vcf_and_export_bundle_round_trip() {
    ...
    let imported = service
        .import_vcf(profile_id, vcf.as_bytes())
        .await
        .unwrap();
