                ...
                match state.list_profiles().await {
                    Ok(list) => {
                        ...
                        if selected_profile().is_none() {
                            let pick = active_profile.id().or_else(|| list.first().map(|p| p.id));
    ...
    rsx! {
        section {
            ...
            if let Some(profile_id) = selected_profile() {
                            ...
                            button {
                                ...
                                onclick: {
                                    ...
                                    move |_| {
                                        ...
                                        spawn(async move {
                                            match state.contact_service.restore_backup(profile_id, &label).await {
                                                Ok(()) => {
                                                    status.set(Some(format!("Restored backup {label}")));
                                                    match state.contact_service.list_backups(profile_id).await {
                                                        Ok(list) => backups.set(list),
                                                        Err(err) => error.set(Some(err.to_string())),
                                                    }
                ...
                p {
                    class: "hint",
                    "Create a profile to manage backups."
                }
            }
        }
    }
