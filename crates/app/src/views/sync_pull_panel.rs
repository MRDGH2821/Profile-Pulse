                ...
                button {
                    ...
                    onclick: {
                        move |_| {
                            ...
                            let changes = sync_prompt.pending_snapshot().map(|(_, c)| c).unwrap_or_default();
                            spawn(async move {
                                let mut sync_prompt = sync_prompt;
                                let Ok(profile) = state.storage.load_profile(prompt_profile_id).await else {
                                    busy.set(false);
                                    return;
                                };
                                ...
                                for target in &changes {
                                    for change in &target.changes {
                                        ...
                                        let local =
                                            state.storage.load_contact(prompt_profile_id, contact_id).await.ok();
                                        match state
                                            .sync_service
                                            .prepare_pull_item(
                                                &profile,
                                            ...
                                            Ok(PullPrepareResult::Apply(remote)) => {
                                                if let Ok(contact) =
                                                    state
                                                        ...
                                                        .await {
                                                    if let Err(err) =
                                                        state.contact_service.update_contact(contact).await {
                                                        error.set(Some(err.to_string()));
                                                    } else {
                                                        applied += 1;
                                                    }
                                ...
                                status.set(
                                    Some(
                                        format!("Pulled {applied} contact(s); {conflict_count} conflict(s) need review")
                                    )
                                );
                                busy.set(false);
                            });
            ...
            div {
                ...
                ul {
                    for conflict in sync_prompt.conflicts_snapshot().iter() {
                        li {
                            "{conflict.local.display_name} ({conflict.target_kind})" Link {
